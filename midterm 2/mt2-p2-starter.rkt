...
;; produce a grade based on students ans matching correct ans
#|
...
(cons String LOCA)    (1) 0      (2)(if (string=? (first losa) (first loca))
                                     (+ 1 (grade (rest losa) (rest loca)))
                                      (if (string=? "blank" (first losa))
                                       (+ 0 (grade (rest losa) (rest loca)))
                                       (- (grade (rest losa) (rest loca)) 1)))

|#
(check-expect (grade empty empty) 0)
(check-expect (grade LOSA2 empty) 0)
(check-expect (grade empty LOCA2) 0)
(check-expect (grade (list "A" "D" "C")
                     (list "A" "B" "C")) 1)
(check-expect (grade (list "A" "D" "C")
                     (list "A" "D" "C")) 3)
(check-expect (grade (list "C" "blank" "A" "B")
                     (list "C" "B" "A" "B")) 3)
(check-expect (grade (list "D" "D" "blank")
                     (list "B" "A" "C")) -2)
(check-expect (grade (list "D" "D" "D")
                     (list "B" "A" "C")) -3)
(check-expect (grade (list "A" "blank" "D")
                     (list "A" "D")) 1)
...
(define (grade losa loca)
  (cond [(or (empty? losa) (empty? loca)) 0]  ;; -> (1)
        [else
         (if (string=? (first losa) (first loca))   ;; -> (2)
             (+ 1 (grade (rest losa) (rest loca)))
             (if (string=? "blank" (first losa))
                 (+ 0 (grade (rest losa) (rest loca)))
                 (- (grade (rest losa) (rest loca)) 1)))]))
