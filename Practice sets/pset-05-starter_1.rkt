...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname pset-05-starter_1) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
...
(check-expect (render-game G1)
              (place-image PADDLE (/ WIDTH 2) PADDLE-CTR-Y
                           (render-balls (cons B1 empty))))
(check-expect (render-game (make-game
              ...
              (place-image PADDLE (/ WIDTH 2) PADDLE-CTR-Y
                           (render-balls
                            (cons B1 (cons
                            (make-ball (/ WIDTH 2) (/ HEIGHT 2) 7 3) empty)))))

(make-ball (/ WIDTH 2) (/ HEIGHT 2) 4 -3)
...
(@htdf next-game)
(@signature Game -> Game)
;; produce next game with paddle and new balls
(check-expect (next-game (make-game empty 20))
              (make-game empty 20)); no balls
(check-expect (next-game (make-game (cons B1 empty) 30))
              (make-game (cons (make-ball (+ (ball-x B1) (ball-dx B1))
                                          (+ (ball-y B1) (ball-dy B1))
                                          (ball-dx B1)
                                          (ball-dy B1))
                               empty) 30)); 1 ball normal
(check-expect (next-game (make-game (cons (make-ball 10 20 4 -5)
                                          (cons (make-ball 30 10 -3 2) empty))
                                    40))
              (make-game (cons (make-ball (+ 10 4) (+ 20 -5) 4 -5)
                               (cons (make-ball (+ 30 -3) (+ 10 2) -3 2)
                                     empty)) 40)); 2 ball normal
(check-expect (next-game (make-game (cons (make-ball (+ LEF 1)
                                                     (+ TOP 3)
                                                     2 -4) empty) 50))
              (make-game (cons (make-ball (+ LEF 1)
                                          (+ TOP 1)
                                          2 4) empty) 50)); 1 ball wall
(check-expect (next-game (make-game (cons (make-ball (- 100 (/ PADDLE-WIDTH 2))
                                                     PADDLE-CTR-Y 1 2)
                                          empty) 100))
              (make-game empty 100)); 1 ball paddle
...
(define (next-game g)
  (game-with-next-balls (game-with-caught-balls (next-balls (game-balls g))
                                                (game-paddle g))
                        (game-paddle g)))
...
(@signature ListOfBall Number -> Game)
;; Produces next game state with given ListOfBall and Number
(check-expect (game-with-next-balls LOB1 50) (make-game LOB1 50))
(check-expect (game-with-next-balls LOB2 50) (make-game LOB2 50))

(@template ListOfBall)

(define (game-with-next-balls lob n)
  (cond [(empty? lob) (make-game empty n)]
        [else (make-game lob n)]))
...
;; Returns lob without all the balls that were caught

(check-expect (game-with-caught-balls empty 20) empty)
(check-expect (game-with-caught-balls LOB2 20) LOB2)
(check-expect (game-with-caught-balls (cons
...
;(define (game-with-caught-balls lob n) lob)

(@template ListOfBall)

(define (game-with-caught-balls lob n)
  (cond [(empty? lob) empty]
        [else
         (if (touch-paddle? (first lob) n)
             (game-with-caught-balls (rest lob) n)
             (cons (first lob) (game-with-caught-balls (rest lob) n)))]))
...
(check-random (handle-mouse G0 100 200 "button-up")
              (make-game empty (/ WIDTH 2)))

(check-random (handle-mouse (make-game (cons (make-ball 10 20 -3 3) empty)
                                       (/ WIDTH 2))
                            100 200 "button-up")
              (make-game (cons (make-ball 10 20 -3 3) empty) (/ WIDTH 2)))
