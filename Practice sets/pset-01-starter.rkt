...
;;   (... i1 i2))
;;
(define IMG1 (rectangle 10 10 "solid" "blue"))
(define IMG2 (rectangle 20 20 "solid" "blue"))

(@htdf taller-image) ;Uncomment this line when you start the problem.
(@signature Image Image -> Image)
;; produces taller image
(check-expect (taller-image IMG1 IMG2) IMG2)
(check-expect (taller-image IMG2 IMG1) IMG2)
;;(define (taller-image img1 img2) img2)

(@template Image)
(define (taller-image img1 img2)
  (if (> (image-height img1)
         (image-height img2))
      img1
      img2))
