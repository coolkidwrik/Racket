...
;; produces String made of letter after applying operations
...
#|
 ...
 _____________________|                |__________________________________    
  (cons "keep"        |                | (string-append (first l)
 Operations)          |                |  (decoder (rest l) (rest ops)))
 _____________________|      ""        |__________________________________
                      |                | (string-append " "
(cons "space"         |                |   (decoder (rest l) (rest ops)))
Operations)           |                |    
 _____________________|                |__________________________________
                      |                |
(cons "remove"        |                | (string-append ""
 Operations)          |                |    (decoder (rest l) (rest ops)))
                      |                |
|#
(check-expect (decoder empty empty) "")
(check-expect (decoder empty (list "remove" "keep"
                                   "space" "keep" "remove" "keep" "space"))
              "") 
(check-expect (decoder (list "a") (list "keep")) "a")
(check-expect (decoder (list "a") (list "space")) " ")
(check-expect (decoder (list "a") (list "remove")) "")
(check-expect (decoder L2 empty) "aBc")
(check-expect (decoder L2 (list "remove" "keep" "space")) "B ")
(check-expect (decoder L2 (list "remove" "keep"
                                "space" "keep" "remove" "keep" "space")) "B ")
(check-expect (decoder (list "i" "k" "l" "h" "a" "m" "k" "s" "a" "r" "m")
                       (list "keep" "remove" "remove" "space" "keep" "keep"
                             "space" "keep" "keep" "remove")) "i am sam")
...
(@template Course ListOfCourse encapsulated try-catch)
...
(@signature Course -> ListOfNatural)
;; produces numbers list that're odd,not pre-requisites,and worth 3 credits
(check-expect (courses-criteria C189) empty)
(check-expect (courses-criteria C100) empty)
(check-expect (courses-criteria (make-course 203 3 LOC0)) (list 203))
(check-expect (courses-criteria C110) (list 203 313 317 313 317 319 311 303))

;(define (courses-criteria c) empty)
...
(define (courses-criteria c)
  (local [
          (define (courses-criteria--course c)
            (if (and (>= (course-credits c) 3)
                     (not (= (modulo (course-number c) 2) 0))
