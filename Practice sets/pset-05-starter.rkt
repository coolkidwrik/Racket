...
(@signature Game -> Game)
;; produce list of balls at their next x, y coordinates
(check-expect (next-game (make-game empty (/ WIDTH 2)))
...
(define (game-with-next-balls g) G0)
...
(@signature ListOfBall -> Game)
;; produce list of balls at their next x, y coordinates
;; !!!


(define (game-with-caught-balls g) G0)

(@template ListOfBall)

#;
(define (game-with-caught-balls lob)
  (cond [(empty? lob) empty]
        [else
         (if (touch-paddle? (first lob))
              (fn-for-ball (first lob))
              (fn-for-lob (rest lob)))]))
...
(check-random (handle-mouse G0 100 200 "button-down")
              (make-game (cons (make-ball 100 200 (- 5 (random 11)) (- 5 (random 11)))
                    empty) (/ WIDTH 2)))

(check-random (handle-mouse (make-game (cons (make-ball 10 20 -3 3) empty)
                                       (/ WIDTH 2))
                         300 100 "button-down")
              (make-game (cons (make-ball 300 100 (- 5 (random 11)) (- 5 (random 11)))
                    (cons (make-ball 10 20 -3 3) empty)) (/ WIDTH 2)))

(check-random (handle-mouse G0 100 200 "button-up")
              (make-game empty (/ WIDTH 2)))

(check-random (handle-mouse (make-game (cons (make-ball 10 20 -3 3) empty) (/ WIDTH 2))
                            100 200 "button-up")
...
(check-expect (handle-key (make-game (cons (make-ball (/ WIDTH 2) (/ HEIGHT 2) 2 4)
                                (cons (make-ball (+ TOP 2) (+ LEF 5) 3 2)
                                      empty)) (/ WIDTH 2))
                          " ")
              (make-game empty (/ WIDTH 2)))

(check-expect (handle-key (make-game (cons (make-ball (/ WIDTH 3) (/ HEIGHT 4) 1 -3)
...
(check-expect (handle-key (make-game (cons (make-ball (/ WIDTH 3) (/ HEIGHT 4) 1 -3)
              ...
              (make-game (cons (make-ball (/ WIDTH 3) (/ HEIGHT 4) 1 -3)
                                (cons (make-ball (+ TOP 5) (+ LEF 2) 3 -2)
                                      empty)) (- (/ WIDTH 2) PADDLE-MOVE-PER-KEY)))
(check-expect (handle-key (make-game (cons (make-ball (/ WIDTH 3) (/ HEIGHT 4) 1 -3)
                                (cons (make-ball (+ TOP 5) (+ LEF 2) 3 -2)
                                      empty)) (/ WIDTH 2))
                          "right")
