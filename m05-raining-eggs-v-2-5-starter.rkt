...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname m05-raining-eggs-v-2-5-starter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require 2htdp/image)
...
(define (render-eggs loe)
  (cond [(empty? loe) MTS]
        [else
         (place-image (egg->image (first loe))
              (render-eggs (rest loe)) MTS)]))


(@htdf egg->image)
(@signature Egg -> Image)
;;turns egg into image
(check-expect (egg->image (make-egg 10 20 30)) 
              (place-image (rotate 30 YOSHI-EGG) 10 20 MTS))

;(define (egg->image e) empty-image)
