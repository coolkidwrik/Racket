...
(define IMG1 (ellipse 60 30 "solid" "red"))
(define IMG2 (ellipse 30 60 "solid" "red"))
;;Image -> Image
(check-expect (boxify IMG1)
              (overlay (rectangle 70 70 "outline" "red")
              IMG1))
(check-expect (boxify IMG2)
              (overlay (rectangle 70 70 "outline" "red")
              IMG2))


;(define (boxify img) empty-image)

(define (boxify img)
  (if (< (image-height img) (image-width img))
      (overlay (rectangle (+ 10 (image-width img)) (+ 10 (image-width img))
                          "outline" "red")
              img)
      (overlay (rectangle (+ 10 (image-height img)) (+ 10 (image-height img))
                          "outline" "red")
              img)))
