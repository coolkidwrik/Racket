...
(@template encapsulated genrec arb-tree try-catch)

(define (solve bd)
  (local [(define (fn-for-bd bd)
            (if  (solved? bd)
                 (list bd)
                 (local [(define try (fn-for-lobd (next-boards bd)))]
                   (if (not (false? try))
                       (cons bd try)
                       false))))
          ...
          (define (fn-for-lobd lobd)
            (cond [(empty? lobd) false]
                  [else
                   (local [(define try (fn-for-lobd (first lobd)))]
                     (if (not (false? try))
                         try
...
(@htdf next-boards)
(@signature Board -> (listof Boards))
;; produce next boards by performing all valid jumps
(check-expect (next-boards BD3) MTB)
(check-expect (next-boards BD4) empty)
(check-expect
 (next-boards BD1)
 (list
  (build-list 15 (lambda (p) (not (or (= p 1)
                                      (= p 3)))))
  (build-list 15 (lambda (p) (not (or (= p 2)
                                      (= p 5)))))))  
...
(define (next-boards bd)
  (local [(define valid? j)
          (... (jump-from j)
               ()
               ())]
    (map do-jumps (filter valid? JUMPS))))
