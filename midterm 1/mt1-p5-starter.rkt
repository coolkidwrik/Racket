...
(@cwl wriksen) ;fill in your CWL here (same as for problem sets)
...
;; @signature and @template tags.
;;
(@htdf render-cat)

(@signature Cat -> Image)
;; place happy/unhappy image of cat onto the MTS at correct position
(check-expect (render-cat HAPPY-LEFT-CAT)
              (place-image HAPPY-CAT-IMAGE LEFT CAT-Y MTS))
(check-expect (render-cat UNHAPPY-LEFT-CAT)
              (place-image UNHAPPY-CAT-IMAGE  LEFT CAT-Y MTS))
(check-expect (render-cat (make-cat MID 2 true))
              (place-image HAPPY-CAT-IMAGE MID CAT-Y MTS))
(check-expect (render-cat (make-cat MID 2 false))
              (place-image UNHAPPY-CAT-IMAGE MID CAT-Y MTS))
(check-expect (render-cat (make-cat RIGHT 2 true))
              (place-image HAPPY-CAT-IMAGE RIGHT CAT-Y MTS))
(check-expect (render-cat (make-cat RIGHT 2 false))
              (place-image UNHAPPY-CAT-IMAGE RIGHT CAT-Y MTS))
...
(define (render-cat c)
  (if (cat-happy? c)
      (place-image HAPPY-CAT-IMAGE (cat-x c) CAT-Y MTS)
      (place-image UNHAPPY-CAT-IMAGE (cat-x c) CAT-Y MTS)))
...
(check-expect (tock-cat HAPPY-LEFT-CAT) (make-cat LEFT  2 true))
(check-expect (tock-cat UNHAPPY-LEFT-CAT) (make-cat LEFT 3 false))
(check-expect (tock-cat (make-cat RIGHT 3 false)) (make-cat RIGHT -3 false))
(check-expect (tock-cat (make-cat MID 3 true)) (make-cat MID 3 true))
(check-expect (tock-cat (make-cat MID 3 false)) (make-cat (+ MID 3) 3 false))
(check-expect (tock-cat (make-cat MID -3 false)) (make-cat (+ MID -3) -3 false))
...
(define (tock-cat c)
  (if (cat-happy? c)
      c
      (if (invert-vel? c)
          (make-cat (cat-x c) (- (cat-unhappy-vel c)) (cat-happy? c)) 
...
(@htdf invert-vel?)
(@signature Cat -> Boolean)
;; produces true if cat needs is at the edge and needs to turn
(check-expect (invert-vel? UNHAPPY-LEFT-CAT) true)
(check-expect (invert-vel? (make-cat MID 3 true)) false)

;(define (invert-vel? c) false)

(@template Cat)

(define (invert-vel? c)
  (or (< (+ (cat-x c) (cat-unhappy-vel c)) LEFT)
