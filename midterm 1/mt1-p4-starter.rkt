...
(@signature ListOfImage Number -> Number)
;; produces the sum of all shapes in the list that have areas bigger than number
(check-expect (sum-areas-greater-than empty 10) 0)
(check-expect (sum-areas-greater-than empty 0) 0)
(check-expect (sum-areas-greater-than (cons I20x30 empty) 600) 0)
(check-expect (sum-areas-greater-than (cons I20x30 empty) 599) 600)
(check-expect (sum-areas-greater-than LOI2 500) 600)
(check-expect (sum-areas-greater-than LOI2 499) 1100)


;(define (sum-areas-greater-than loi n) 0) ;stub
