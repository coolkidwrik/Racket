...
(define (max-num-repeats los)
  ;;INVARIENT: rsf; Natural, number of repeats for current string so far
  ;;INVARIENT: max; Natural max rsf recorded so far
  ;;INVARIENT: prev; previous string
  (local [(define (fn-for-los los rsf max prev)
            (cond [(empty? los) max]
                  [else
                   (if (equal? (first los) prev)
                       (fn-for-los (rest los)
                                   (+ 1 rsf)
                                   (if (> (+ 1 rsf) max)
                                       (+ 1 rsf)
                                       max)
                                   (first los))
                       (fn-for-los (rest los) 1 max (first los)))]))]
    
    (fn-for-los los 0 0 (if (empty? los)
                            empty
                            (first los)))))
...
(define (list-range loi)
  ;;INVARIENT: max; Number, max number in the list so far
  ;;INVARIENT: min; Number, min number in the list so far
  (local [(define (fn-for-lon lon max min)
            (cond [(empty? lon) (- max min)]
                  [else
                   (fn-for-lon (rest lon)
                               (if (> (first lon) max)
                                   (first lon)
                               ...
                               (if (< (first lon) min)
                                   (first lon)
                                   min))]))]
    (fn-for-lon loi (first loi) (first loi))))
...
(define (in-alphabetical-order? los)
  (local [(define (fn-for-los los prev rsf)
            (cond [(empty? los) rsf]
                  [else
                   (fn-for-los (rest los)
                               (first los)
                               (and (string-ci<=? prev (first los))
                                    rsf))]))]
    (fn-for-los los (if (empty? los)
                        empty
                        (first los)) true)))
...
(define (filter-credits-course-num c0 n cred)
  ;;INVARIENT: todo; (listof Course),list of courses yet to visit
  ;;INVARIENT: rsf; (listof Courses),courses that match description so far
  (local [(define (fn-for-course c todo rsf)
            (fn-for-loc (append (course-dependents c) todo)
                        (if (and (>= (course-number c) n)
...
(define (max-credit-path c0)
  ;;INVARIENT: todo; (listof Course),list of courses yet to visit
  ;;INVARIENT: path; (listof Course), path to that course
  ;;INVARIENT: p-wl; (listof (listof Course)), list of paths
  ;;INVARIENT: max; Natural, highest sum of courses so far
  ;;INVARIENT: rsf; (listof Course), highest cred path so far
  (local [(define (fn-for-course c todo p p-wl max rsf)
            (local [(define subs (course-dependents c))
                    (define cred (foldr + 0 (map course-credits p)))]
              
              (fn-for-loc (append subs todo)
                          (append (map (λ (s) (append p (list s))) subs) p-wl)
                          (if (> cred max)
                              cred
                              max)
                          (if (> cred max)
            ...
            (define (fn-for-loc todo p-wl max rsf)
              (cond [(empty? todo) (map course-number rsf)]
                    [else
                     (fn-for-course (first todo)
                                    (rest todo)
                                    (first p-wl)
...
(fn-for-course c0 empty (list c0) empty 0 empty)))  
