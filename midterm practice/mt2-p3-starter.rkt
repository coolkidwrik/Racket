...
;;produces value of node if exists, else false
(check-expect (find-val TT1 1) false)
(check-expect (find-val TT2 7) false)
(check-expect (find-val TT2 2) "two")
(check-expect (find-val TT3 7) false)
(check-expect (find-val TT3 3) "three")
(check-expect (find-val TT3 6) "six")

;(define (find-val tt n) false)

(@template TTree try-catch)

(define (find-val tt n)
  (cond [(false? tt) false]
        [else
         (if (= (node-k tt) n)
             (node-v tt)
             (local [(define try (find-val (node-lef tt) n))]
               (if (not (false? try))
                   try
                   (local [(define try (find-val (node-mid tt) n))]
                     (if (not (false? try))
                         try
                         (find-val (node-rig tt) n))))))]))
