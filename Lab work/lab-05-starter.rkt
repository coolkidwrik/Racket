...
(@htdf place-row)
(@signature Natural Image -> Image)
;; produces one row of the pyramid
(check-expect (place-row 1 SQUARE) SQUARE)
(check-expect (place-row 2 COOKIES)
              (beside COOKIES COOKIES))
(check-expect (place-row 3 COOKIES)
              (beside COOKIES COOKIES COOKIES))

;(define (place-row n img) img)

(@template Number)

(define (place-row n img)
  (cond [(zero? n) empty-image]
        [else
         (beside img                   
              (place-row (sub1 n) img))]))
...
;; swaps the first and second elements of the list
(check-expect (swap-blob "bubble" (cons "solid" (cons "bubble" empty)))
              (cons "bubble" (cons "solid" (cons "bubble" empty))))
(check-expect (swap-blob "solid" (cons "solid" (cons "bubble" empty)))
              (cons "solid" (cons "solid" (cons "bubble" empty))))
(check-expect (swap-blob "solid" (cons "bubble" (cons "bubble" empty)))
              (cons "bubble" (cons "solid" (cons "bubble" empty))))


;(define (swap-blob b lob) lob)

(@template ListOfBlob)

(define (swap-blob b lob)
