...
(define CH3 (make-chst (list) (list O R V)))
...
;; produces a list of clown from choice state provided that meets crit
(check-expect (solve (make-chst (list S R P O M) (list))) 
              (list S R P O M))
(check-expect (solve CH0) (list S R P O M))

;(define (solve chst) empty)

(@template bin-tree genrec try-catch)


(define (solve c)
  (cond [(empty? (chst-options c))
         (if (all-crits? (chst-picked c))
             (chst-picked c)
             false)]
        [else
         (local [(define try (solve (next-left-node c)))]
           (if  (not (false? try))
                try
                (local [(define try (solve (next-right-node c)))]
                  (if  (not (false? try))
                       try
                       false))))]))
...
(@htdf solved?)
(@signature ChoiceState -> Boolean)
;; produces true if all crit are met and nothing left to choose
(check-expect (solved? CH0) false)
(check-expect (solved? CH1) false)
(check-expect (solved? (make-chst (list S R P O M) (list))) true)

;(define (solved? c) false)

(@template ChoiceState)

(define (solved? c)
  (and (all-crits? (chst-picked c))
       (empty?     (chst-options c))))



(@htdf next-right-node)
(@signature ChoiceState -> ChoiceState)
;; produces next node after skipping first option
(check-expect (next-right-node CH0)
              (make-chst (list) (list N O P R S V)))

;(define (next-right-node chst) chst)

(@template ChoiceState)

(define (next-right-node c)
  (make-chst (chst-picked c)
             (rest (chst-options c))))
...
(@htdf next-left-node)
(@signature ChoiceState -> ChoiceState)
;; produces next node after choosing first option
(check-expect (next-left-node CH0)
              (make-chst (list M) (list N O P R S V))) 

;(define (next-right-node chst) chst)

(@template ChoiceState)

(define (next-left-node c)
  (make-chst (cons (first (chst-options c)) (chst-picked c))
             (rest (chst-options c)))) 
