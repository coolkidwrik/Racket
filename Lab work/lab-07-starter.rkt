...
;(bitmap/url
; "https://edx-course-spdx-kiczales.
;s3.amazonaws.com/HTC/lab/lab-07-yes-no-tree.png")
...
#|
 ...
 ____V_____________\__|________________|__________________________________
                      |                |
   String             |  (string? ynt) |        false
                      |                |    
 _____________________|                |__________________________________    
                      |                |
(make-yntree String   |  (string? ynt) | (or (contains-n-path? (yntree-y ynt)  
 YesNoTree            |                |   (sub1 n))
 YesNoTree)           |                |(contains-n-path? (yntree-n ynt)
                                            (sub1 n))
|#
(check-expect (contains-n-path? YNT0 0) true)
(check-expect (contains-n-path? YNT1 0) (string? YNT1))
(check-expect (contains-n-path? YNT0 (add1 0)) false)
(check-expect (contains-n-path? YNT1 (add1 0)) true)
(check-expect (contains-n-path? YNT1 (add1 1)) true)
(check-expect (contains-n-path? YNT1 5) false)
...
;;produce list of matches given lists' of players
#|
 CROSS PRODUCT OF TYPE-COMMENTS TABLE
 ...
 _____________________|________________|__________________________________    
                      |                |
(cons Player          |        C       |(list (make-match (first lop1) 
 ListOfPlayer)        |                | (first lop2))
                                         (set-matches (rest lop1)
                                           (rest lop2)))
C = (or (empty? lop1) (empty? lop2))
