...
(@htdf num-lr)
;(@signature _ _ _ -> _)
;(@signature _ _ _ -> Integer)
;(@signature (Integer -> Integer) (Integer -> Integer) _ -> Integer)
;(@signature (Integer -> Integer) (Integer -> Integer) _ -> Integer)
;(@signature (Integer -> Integer) (Integer -> Integer) (listof _) -> Integer)
(@signature (Integer -> Integer) (Integer -> Integer) Path -> Integer)

;; absract function for path
(check-expect (num-lr add1 sub1 empty) 0)
(check-expect (num-lr sub1 add1 empty) 0)
...
(define (num-lr fn1 fn2 p)
  (cond [(empty? p) 0]
        [(string=? (first p) "L") (fn1 (num-lr fn1 fn2 (rest p)))]
        [(string=? (first p) "R") (fn2 (num-lr fn1 fn2 (rest p)))]))
...
;; produces the length of the path trough the tree
(check-expect (path-length empty) 0)
(check-expect (path-length (list "R")) 1)
(check-expect (path-length (list "R" "R")) 2)
...
(@htdf fold-path)
;(@signature _ _ _ _ -> _)
;(@signature (_ _ -> _) (_ _ -> _) _ _-> Y)
;(@signature (_ _ -> _) (_ _ -> _) _ Path -> Y)
(@signature (X -> X) (X -> X) X Path -> X)
;; fold function for path
(check-expect (fold-path string-append string-append "" empty) "")



#;
(define (fold-path fn p)
  (cond [(empty? p) b]
        [else
         (fn (first p)
             (fold-path fn (rest p)))]))
...
(@template Path)
(define (fold-path fn1 fn2 b p)
  (cond [(empty? p) b]
        [(string=? (first p) "L") (fn1 (fold-path fn1 fn2 b (rest p)))]
        [(string=? (first p) "R") (fn2 (fold-path fn1 fn2 b (rest p)))]))
...
;; produces a single string that concatenates all strings in path
(check-expect (path-string empty) "")
(check-expect (path-string (list "L")) "L")
(check-expect (path-string (list "L" "R" "L")) "LRL")
(check-expect (path-string (list "R" "L" "R")) "RLR")

(@template use-abstract-fn)
(define (path-string p)
  (local [(define (c1 result) (string-append "L" result))

          (define (c2 result) (string-append "R" result))]
  (fold-path c1 c2 "" p)))
...
(define C0 (make-class "CPSC229" "202" 2 4 40 84 "CMPTNL BSKT WVNG")) ; < 70
(define C1 (make-class "CPSC259" "201" 2 4 190 188 "DTA&ALG ELEC ENG")) ; >70
(define C2 (make-class "CPSC400" "123" 1 0 190 188 "SNRITIS FOR MJRS")) ; > 70
(define C3 (make-class "CPSC401" "123" 1 0 70 100 "SNRITIS FOR MJRS")) ; = 70
(define C4 (make-class "CPSC402" "123" 1 0 71 100 "SNRITIS FOR MJRS")) ; =71
(define C5 (make-class "CPSC403" "123" 2 0 69 100 "SNRITIS FOR MJRS")) ; =69
(define C6 (make-class "CPSC404" "123" 2 0 190 188 "SNRITIS FOR MJRS"))
(define C7 (make-class "CPSC405" "123" 2 0 70 100 "SNRITIS FOR MJRS")) 
...
;; produces number of classes from term 2 and enrollment > 70
(check-expect (popular-spring-class-count empty) 0)
(check-expect (popular-spring-class-count (list C0 C1 C2)) 1)
(check-expect (popular-spring-class-count (list C1 C6 C3)) 2)
(check-expect (popular-spring-class-count (list C3)) 0)
(check-expect (popular-spring-class-count (list C4)) 0)
(check-expect (popular-spring-class-count (list C5)) 0)
(check-expect (popular-spring-class-count (list C6)) 1)
(check-expect (popular-spring-class-count (list C7)) 0)


(@template fn-composition use-abstract-fn)

(define (popular-spring-class-count loc)
  (local [(define (is-pop-spring? c) (and (< 0.7
                                              (/ (class-enrolled c)
                                                 (class-capacity c)))
                                          (= (class-term c) 2)))]
    (length (filter is-pop-spring? loc))))
