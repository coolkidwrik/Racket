...
(@signature Node -> (listof String) or false)
;; produces a listof node names from start to destination with ascending numbers, else false
(check-expect (find-increasing-path "A" "A") (list "A"))
(check-expect (find-increasing-path "G" "A") false)
...
;(define (find-increasing-path n1 n2) false)

(@template genrec Node (listof Node) accumulator)

(define (find-increasing-path n1 n2)
  ;; Termination argument:
  ;;
  ;;INVARIANT: todo; (listof Node) left to visit
  ;;INVARIANT: p; (listof String) already visited
  ;;INVARIANT: p-wl; (listof (listof String)) path to each node
  ;;INVARIANT: prev; Natural, last highest number
  (local [(define (fn-for-node n todo p p-wl prev)
            (cond [string=? (node-name n) n2]
                  [(member n p) false]
                  [else
                   (fn-for-los (append (node-nexts n) todo)
                        (... p)
                        (... p-wl)
                        (... prev))]))
          
          (define (fn-for-los todo p p-wl prev)
            (cond [(empty? todo) false]
                  [else
                   (fn-for-string (first todo)
                                  (rest todo)
                                  (first (p-wl))
                                  (rest p-wl)
                                  prev)]))

          (define (fn-for-string s todo p p-wl prev)
            (fn-for-node (lookup-node s)
                         todo
                         p
                         p-wl
...
(define (find-increasing-path n1 n2)
  ...
  ;; 
  ;;INVARIANT: todo; (listof Node) left to visit
  ;;INVARIANT: path; (listof Node) already visited
  ;;INVARIANT: prev; Natural, last highest number
  (local [(define (fn-for-node n todo path prev)
            (cond [(or (member n path)
                       (< (node-num n) prev)) false]
                  [(equal? (node-name n) n2) (reverse (cons (node-name n) path))]
                  [else
                   (fn-for-los (append (node-nexts n) todo) 
                               (cons (node-name n) path)
                               (node-num n))]))
          
          (define (fn-for-los todo path prev)
            (cond [(empty? todo) false]
                  [else
                   (local [(define try (fn-for-string (first todo) 
                                                      (rest todo)
                                                      path
                                                      prev))]
                     (if (not (false? try))
                         try
                         (fn-for-los (rest todo)
                                     path
                                     prev)))]))

          (define (fn-for-string s todo path prev)
            (fn-for-node (lookup-node s)
                         todo
                         path
                         prev))]
    
    (fn-for-string n1 empty empty -1)))

#;
(cond [(equal? (node-name n) n2) (reverse (cons (node-name n) path))]
      [(or (member n path)
           (> prev (node-num n))) false]
      [else
       (fn-for-los (append (node-nexts n) todo)
                   (cons (node-name n) path)
                   (node-num n))])
