...
;; produce the longest sequence of consequtive integers
...
;(define (longest-sequence loi) empty)

(@template (listof Number))

(define (longest-sequence loi0)
  ;;INVARIENT: prev; Number, previous number in the list
  ;;INVARIENT: rsf1; (listof Number), list of consecutive numbers so far
  ;;INVARIENT: rsf2; (listof Number), longest list of consecutive numbers so far
  (local [(define (fn-for-loi loi prev rsf1 rsf2)
            (cond [(empty? loi) rsf2]
                  [else
                   (if (= (first loi) (add1 prev))
                       (fn-for-loi (rest loi)
                                   (first loi)
                                   (append rsf1 (list (first loi)))
                                   (if (> (length (cons (first loi) rsf1))
                                          (length rsf2))
                                       (append rsf1 (list (first loi)))   
                                       rsf2))
                       (fn-for-loi (rest loi)
    ...
    (fn-for-loi loi0 (if (empty? loi0)
                         empty
                         (sub1 (first loi0))) empty empty)))
