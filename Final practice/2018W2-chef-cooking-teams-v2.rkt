...
(@htdf select-teams)
(@signature (listof Chef) -> (list (listof Chef) (listof Chef)) or false)
;; produce two teams of chefs that have the same years of experience > 10
(check-expect (select-teams empty) false)
(check-expect (select-teams (list C1 C2 C3 C4)) false)
(check-expect (select-teams (list C8 C9 C10 C11 C12)) (list
                                                       (list
                                                        (make-chef "Andy" 4 "sushi")
                                                        (make-chef "Rick" 7 "sushi"))
                                                       (list
                                                        (make-chef "Tina" 2 "burgers")
                                                        (make-chef "Jill" 9 "ramen"))))  
(check-expect (select-teams (list C11 C12 C8 C5)) (list
                                                   (list
                                                    (make-chef "Tina" 2 "burgers")
                                                    (make-chef "Jill" 9 "ramen"))
                                                   (list
                                                    (make-chef "Ken" 4 "desserts")
                                                    (make-chef "Rick" 7 "sushi"))))


(define (select-teams loc)
  (local [(define (red-chef-no o)
            (foldr + 0 (map chef-experience (organizer-red o)))) 
          (define (blue-chef-no o)
            (foldr + 0 (map chef-experience (organizer-blue o))))

          (define (solved? o)
            (and (>= (red-chef-no o) 10)
                 (= (red-chef-no o) (blue-chef-no o))))

          ;;INVARIENT: todo;(listof Organizer) worklist of nodes to check
          (define (solve todo)
            (cond [(empty? todo) false]
                  [else
                   (local [(define o (first todo))]
                     (cond [(solved? o) (list (organizer-red o) (organizer-blue o))] 
                           [(empty? (organizer-undecided o)) (solve (rest todo))]
                           [else
                            (local [(define pick-red
                                      (make-organizer (cons (first (organizer-undecided o))
                                                            (organizer-red o))
                                                      (organizer-blue o)
                                                      (rest (organizer-undecided o)))) 
                                    (define pick-blue
                                      (make-organizer (organizer-red o)
                                                      (cons (first (organizer-undecided o))
                                                            (organizer-blue o))
                                                      (rest (organizer-undecided o))))
                                    (define skip
                                      (make-organizer (organizer-red o)
                              ...
                              (solve (append (list pick-red pick-blue skip) (rest todo))))]))]))] 

    (solve (list (make-organizer empty empty loc)))))
...
(define (select-teams loc)
  (local [(define (red-chef-no o)
          ...
          (define (solve o)
            (cond [(solved? o) (list (organizer-red o) (organizer-blue o))]
                  [(empty? (organizer-undecided o)) false]
                  [else
                   (local [(define pick-red
                             (make-organizer (cons (first (organizer-undecided o))
                                                   (organizer-red o))
                           ...
                           (define skip
                             (make-organizer (organizer-red o)
                                             (organizer-blue o)
                                             (rest (organizer-undecided o))))
                           (define try1 (solve pick-red))
                           (define try2 (solve pick-blue))]
                     (if (not (false? try1))
                         try1
                         (if (not (false? try2))
                             try2
    ...
    (solve (make-organizer empty empty loc))))
