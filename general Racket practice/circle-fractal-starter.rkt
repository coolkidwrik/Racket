...
(@assignment bank/genrec-p1)
(@cwl wriksen)

(@problem 1)
;; Design a function that will create the following fractal:
;;
;; https://cs110.students.cs.ubc.ca/bank/circle-fractal.PNG
;; 
...
;; produce image of circle fractal of size n
(check-expect (circles TRIVIAL-SIZE) (circle TRIVIAL-SIZE "solid" "blue"))
(check-expect (circles 10) (local [(define next-shape-above
                                     (above (circle (* 10 STEP) "solid" "blue")
                                            (circle 10 "solid" "blue")
                                            (circle (* 10 STEP) "solid" "blue")))]
                             (overlay next-shape-above (rotate 90 next-shape-above))))
                                     

;(define (circles n) empty-image)

(define (circles n)
  (cond [(<= n TRIVIAL-SIZE) (circle TRIVIAL-SIZE "solid" "blue")]
        [else
         (local [(define (draw-leaf))])]))
